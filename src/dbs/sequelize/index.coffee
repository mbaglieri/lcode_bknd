...
seq       = new Sequelize(config.sequelize.database, config.sequelize.username, config.sequelize.password, config.sequelize.options)
db        = {}
...
seq.authenticate().then ((err) ->
  log.i 'DATABASE- Connection has been established successfully.'
  return
), (err) ->
  log.i err.stack
  log.i 'DATABASE- Unable to connect to the database:', err
  return
