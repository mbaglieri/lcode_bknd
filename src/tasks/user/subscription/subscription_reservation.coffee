...
@reservation_buy = (qtask, worker_id) ->
  q_us_exec_l   = []
  us_sub_exec_l = []
  Promise.try ->
  ...
  .then (user_subscription) ->
    Promise.all(_buy_subsc user, qtask, @worker for user in user_subscription)
  ...
  .then (qtask_users) ->
    ...
    Promise.all(_set_qtask_subscription_buy q_user, qtask, @worker for q_user in qtask_users)
  .then (worker_s) ->
...
_set_qtask_subscription_buy = (qtask_user, qtask, worker) ->
  Promise.try ->
...
_buy_subsc = (user_subscr, qtask, worker) ->
  Promise.try ->
